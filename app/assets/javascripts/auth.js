...
// your own client id.
var OAUTH2_CLIENT_ID = '345770497204.apps.googleusercontent.com';
var OAUTH2_SCOPES = [
...
googleApiClientReady = function() {
  console.log("starting googleApiClientReady");
...
function handleAuthResult(authResult) {
  console.log("starting handleAuthResult");
  if (authResult) {
    console.log("authResult was successful");
    // Auth was successful; hide the things related to prompting for auth and show the things
    // that should be visible after auth succeeds.
    $('.pre-auth').hide();
    loadAPIClientInterfaces();
  } else {
    console.log("authResult was not successful, going to click login");
    ...
    //forcing api load anyway
    loadAPIClientInterfaces();
    $('#login-link').click(function() {
      console.log("login-link click function");
      gapi.auth.authorize({
        client_id: OAUTH2_CLIENT_ID,
        scope: OAUTH2_SCOPES,
        immediate: false
